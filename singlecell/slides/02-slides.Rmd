...
- Ambient expression $A_g$ for gene $g$ is sum of counts $y_{gb}$ over all barcodes $b$:
$$ A_g = \sum_b y_{gb}  $$
...
- DESeq2 library size factors use the median ratio method (Anders and Huber 2010 (https://doi.org/10.1186/gb-2010-11-10-r106)): $$ \hat{s}_b = \text{median}_g \frac{y_{gb}}{(\prod_{j=1}^B y_{gj})^{1/B}} $$
- assumes majority of genes are not differentially expressed

## Median ratio method in single-cell

- Consider this quantity in single-cell: $$ \hat{s}_b = \text{median}_g \frac{y_{gb}}{\color{red}{(\prod_{j=1}^B y_{gj})^{1/B}}} $$
- For every gene $g$ where $y_{gb} = 0$ for any $b$, the highlighted quantity will be undefined
- This means that the quantity is only computed **for genes that are nonzero in every cell**
- As a result, log CPM normalization commonly used for single-cell: $$ log( \frac{y_{gb}*10^6}{\sum_b y_{gb}} + 1) $$
...
- Fits negative binomial generalized linear model for groups of genes with similar geometric means: $$ log(E(y_{gb})) = \beta_0 + \beta_1 log_{10}(t_b), $$ where $t_b = \sum_g y_{gb}$ is the library size for cell $b$
...
- limma: Ritchie et al. 2015 (https://doi/org/10.1093/nar/gkv007) 
  + fit a linear model to log-normalized counts to estimate batch effects $\beta_2$ and effects of interest $\beta_1$:  $$ log(y_{gb}) = \beta_0 + \beta_1 x_b + \beta_2 z_{b} $$
  + remove batch effect: $$ log(y_{gb}) - \beta_2 z_{b} $$
...
## MNNcorrect can remove align heterogeneous batches
...
- Not necessary if data was already log CPM normalized
...
smoothScatter(rowMeans(vstc), rowVars(vstc), 
     xlab="Mean DESeq2::vst expression", 
     ylab="Variance of DESeq2::vst expression")
```
...
```
Andrews & Hemburg 2019 (https://doi-org/10.12688/f1000research.16613.2))
