...
As recommended, also adjust for the number of expressed genes (`total_features_by_counts` in `colData`). Hint: this requires input of a model matrix type formula to the `zlm()` function - `~ Oncogene`. Then run `lrTest` on the output of `zlm()`. The relevant p-values are in the third slot of the third dimension of the results (`[,3,3]`).

```{r}
zlmCond <- zlm(~Oncogene, sca)
lrt <- lrTest(zlmCond, "Oncogene")
sum(p.adjust(lrt[,3,3]) < 0.01)

table(p.adjust(lrt[,3,3]) < 0.01, results(dd)$combined.pvalue.adj < 0.01)
```
