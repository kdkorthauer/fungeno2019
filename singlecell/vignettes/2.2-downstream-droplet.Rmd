...
```{r}
# your code here
snn.gr <- buildSNNGraph(sce, use.dimred="PCA", k = 40)
clusters <- igraph::cluster_walktrap(snn.gr)
sce$Cluster2 <- factor(clusters$membership)
...
```{r}
# your code here
cluster.mod <- clusterModularity(snn.gr, sce$Cluster2, get.values=TRUE)
log.ratio <- log2(cluster.mod$observed/cluster.mod$expected + 1)
...
sum(p.adjust(out[[1]]$p.value) < 0.05)
```
