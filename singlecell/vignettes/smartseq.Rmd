...
In this section, we use a relatively simple dataset [@lun2017assessing] to introduce most of the concepts of scRNA-seq data analysis.
...
We can look at the same quantity, but averaging just over non-zero cells. Here we
manually calculate the average of nonzero cells.

```{r nexprshist416c, fig.cap="The number of cells expressing each gene in the 416B dataset, plotted against the log-average nonzero count. Intensity of colour corresponds to the number of genes at any given location."}
ave.nz.counts <- apply(counts(sce), 1, function(x) mean(x[x>0]))
smoothScatter(log10(ave.nz.counts), num.cells, ylab="Number of cells", 
    xlab=expression(Log[10]~"average nonzero count"))
```
