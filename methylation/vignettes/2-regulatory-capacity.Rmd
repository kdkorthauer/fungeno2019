...
```{r, annotatr-dmrseq}
regions.sig <- regions[regions$qval < 0.05,]
...
From this figure we see that more genes with increased methylation percentage are transcriptionally repressed.
...
dmrsa <- data.frame(dmrs@listData) %>% 
  ...
  mutate(ptcol2 = ifelse(sig, "Differentially Expressed", 
         ...
         ptcol3 = ifelse(qval > 0.05, "Not significant (DMR)", 
